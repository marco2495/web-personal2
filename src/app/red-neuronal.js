...
const RedNeuronal = () => {
    useEffect(() => {
        ...
        for (var i = 0; i < points.length; i++) {
            ...
            if (distanceToMouse <= settings.attractionRange) {
                var displacementFactor = (Math.cos(distanceToMouse / settings.attractionRange * Math.PI) + 1) / 2 * settings.attractionFactor;
    ...
    }, []); // El array vacío como segundo argumento significa que este efecto se ejecutará solo al montar y desmontar el componente
